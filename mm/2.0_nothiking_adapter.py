...
def parse_think_answer_string(text_string):
    """
    从包含 <think>...</think> 和 <answer>...</answer> 标签的字符串中提取内容。

    Args:
        text_string (str): 包含标签的输入字符串。

    Returns:
        tuple: 一个包含 (think_content, answer_content) 的元组。
               如果某个标签未找到，对应的内容将是 None。
    """
    think_content = None
    answer_content = None

    # 使用正则表达式查找 <think> 标签内的内容
    # (.*?) 是一个非贪婪匹配，匹配 <think> 和 </think> 之间的任何字符（包括换行符，因为有 re.DOTALL）
    think_match = re.search(r"<think>(.*?)</think>", text_string, re.DOTALL)
    if think_match:
        think_content = think_match.group(1).strip() # .strip() 用于去除可能存在的前后空白

    # 使用正则表达式查找 <answer> 标签内的内容
