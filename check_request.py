        ...
        for taskinfo in taskqueue.values():
            ...
            if evalinfo["status"] in ["S","DI"]:
                ...
                if taskinfo["retry"] >= 10:
                    ...
                    resp = batchresumption(taskinfo["batch_id"],taskinfo["eval_model"], taskinfo["model"], taskinfo["url"], taskinfo["tokenizer"], taskinfo["api_key"], taskinfo["batch_size"], taskinfo["num_concurrent"], taskinfo["num_retry"],taskinfo["max_gen_toks"], taskinfo["gen_kwargs"], taskinfo.get("mode","FlagRelease"), taskinfo.get("region","bj"))
