...
# 异步请求单个URL
#async
def fetch_url(request_id,taskqueue, taskinfo, mode, region, special_event) -> dict:
    #global taskqueue
    try:
        print("taskinfo",taskinfo)
        response = submit_evaluation(taskinfo.eval_model, taskinfo.model, taskinfo.eval_url, taskinfo.tokenizer, taskinfo.api_key, taskinfo.batch_size, taskinfo.num_concurrent, taskinfo.num_retry, taskinfo.max_gen_toks, taskinfo.gen_kwargs, mode, region, special_event, taskinfo.chip)
...
def mmfetch_url(request_id, taskqueue, taskinfo, mode, region, special_event) -> dict:
    try:
        print(taskinfo)
        ...
        print(taskinfo)
        response = submit_mm_evaluation(taskinfo.eval_model, taskinfo.eval_url, taskinfo.api_key, taskinfo.model, taskinfo.batch_size, taskinfo.num_concurrent, taskinfo.num_retry, taskinfo.max_gen_toks, taskinfo.thinking, taskinfo.retry_time, mode, region, special_event, taskinfo.chip)
        if response["err_code"] == 0:
...
async def compare_urls(request: CompareRequest):
    try:
        ...
        if Domain == "NLP":
            ...
            tasks = fetch_url(request_id, taskqueue, request.eval_infos[0], mode, region, special_event)
        else:
            #tasks = [mmfetch_url(request_id, taskqueue, taskinfo) for taskinfo in request.eval_infos]
            tasks = mmfetch_url(request_id, taskqueue,request.eval_infos[0], mode, region, special_event)
        print("tasks", tasks)
...
async def resume_evaluation(request: DiffRequest):
    ...
    resp = batchresumption(taskinfo["batch_id"],taskinfo["eval_model"], taskinfo["model"], taskinfo["url"], taskinfo["tokenizer"], taskinfo["api_key"], taskinfo["batch_size"], taskinfo["num_concurrent"], taskinfo["num_retry"],taskinfo["max_gen_toks"], taskinfo["gen_kwargs"])
        ...
        for detail in details:
            if detail["status"] == "S":
                ...
                if request.domain == "NLP":
                    ...
                    loginfos = mmbatchlog(taskinfo["batch_id"],taskinfo["eval_model"], taskinfo["model"], taskinfo["url"], taskinfo["tokenizer"], taskinfo["api_key"], taskinfo["batch_size"], taskinfo["num_concurrent"], taskinfo["num_retry"],taskinfo["max_gen_toks"], taskinfo["gen_kwargs"], taskinfo.get("mode","FlagRelease"))
                print("loginfos", loginfos)
